...
void AddVerticesProcessor::doProcessWithRef(const cpp2::AddVerticesRequest& req) {
  const auto& partVertices = req.get_parts();
  const auto& propNamesMap = req.get_prop_names();
  for (auto& part : partVertices) {
    ...
    // graph to ensure that the vertexId only appears once in the list
    for (auto& vertex : vertices) {
      auto vid = vertex.get_id().getStr();
      const auto& newTags = vertex.get_tags();
      ...
      auto l = std::make_tuple(spaceId_, partId, vid);
      if (std::find(dummyLock.begin(), dummyLock.end(), l) == dummyLock.end()) {
        if (!env_->verticeEdgesML_->try_lock(l)) {
          LOG(ERROR) << folly::format("The vertex locked : vid {}", vid);
      ...
      // 2) Add vertex ref data
      auto vertexRefRet = findVertexRef(partId, vid);
      if (!nebula::ok(vertexRefRet)) {
        code = nebula::error(vertexRefRet);
        break;
      }
      auto val = nebula::value(vertexRefRet);
      std::set<TagID> allTagIds;
      if (!val.empty()) {
        allTagIds = NebulaKeyUtils::parseVertexRefVal(val);
      }
      allTagIds.insert(tagIds.begin(), tagIds.end());
      auto vertexRefKey = NebulaKeyUtils::vertexRefKey(spaceVidLen_, partId, vid);
      auto vertexRefVal = NebulaKeyUtils::vertexRefVal(allTagIds);
      batchHolder->put(std::move(vertexRefKey), std::move(vertexRefVal));
    }

    if (code != nebula::cpp2::ErrorCode::SUCCEEDED) {
      env_->verticeEdgesML_->unlockBatch(dummyLock);
      handleAsync(spaceId_, partId, code);
      continue;
    }
    auto batch = encodeBatchValue(batchHolder->getBatch());
    DCHECK(!batch.empty());
    nebula::MemoryLockGuard<VEMLI> lg(
        env_->verticeEdgesML_.get(), std::move(dummyLock), false, false);
    env_->kvstore_->asyncAppendBatch(
        spaceId_,
        partId,
        std::move(batch),
        [l = std::move(lg), partId, this](nebula::cpp2::ErrorCode retCode) {
          UNUSED(l);
...
ErrorOr<nebula::cpp2::ErrorCode, std::string> AddVerticesProcessor::findVertexRef(
